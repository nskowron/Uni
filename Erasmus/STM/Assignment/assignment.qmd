...
\begin{matrix}
    ...
    CI = c^T\widehat{\beta} \pm \widehat{SE}_{c^T\widehat{\beta}} \cdot t_{n - p;1 - \frac{\alpha}{2}}
\end{matrix}
$$
...
predictors nor responses we can use the output of the program to calculate it 
differently:
...
\left[\begin{array}{cc|cc}
    1 & \bar{x} & \frac{1}{n} & 0 \\
    0 & \sum{x_i(x_i - \bar{x})} & -\bar{x} & 1
\end{array}\right]
$$
$$
\left[\begin{array}{cc|cc}
    1 & \bar{x} & \frac{1}{n} & 0 \\
    0 & 1 & \frac{-\bar{x}}{\sum{x_i(x_i - \bar{x})}} & \frac{1}{\sum{x_i(x_i - \bar{x})}}
\end{array}\right]
$$
...
$\frac{\sigma^2}{\sum{x_i(x_i - \bar{x})}}$
