...
Here the situation is slightly different. The test statistic is much lower ($2.5844$), 
...
would be able to say that the full model can be reduced to Model B.

## 5. Explained Variation

Since I'm choosing to work with significance level of $0.05$ and therefore rejecting 
the hypothesis that the full model can be reduced to either of the above - I must 
assume that my "final model" is the full one. \newline

- The amount of variation explained is $\frac{SS_{tot} - RSS}{SS_{tot}} = \frac{RSS_0 - RSS}{RSS_0}$ which is the $R^2$ value.

We can see it already in the full model summary at the beginning - it's around $0.74$, 
but we can make a quick sanity check:
...
- Variation explained by the model: $\sim0.74$
- Residual Sum of Squares: $\sim276$
- Total Sum of Squares: $1057$
- Absolute reduction: $1057 - 276 = 781$

which agrees with the output of our first `anova` test, where we can also find 
...
\begin{matrix}
    \begin{matrix}
        \textbf{X} = 
            \begin{bmatrix}
                1 & \textbf{0} & \textbf{0} & \textbf{0} & X_1 \\
                \vdots & \textbf{1} & \textbf{0} & \textbf{0} & \vdots \\
                \vdots & \textbf{0} & \textbf{1} & \textbf{0} & \vdots \\
                1 & \textbf{0} & \textbf{0} & \textbf{1} & X_n
            \end{bmatrix}
        & \beta = 
            \begin{bmatrix}
                \mu \\
...
In order to do that we need to take half of the observations for $x = -2$ and the 
other half for $x = 2$.
