...
However, the current tableau is clearly not feasible 
...
of the formulation we're in, so they can either be binary or continuous, 
as long as $x_{ik}$ are binary. \newline
...
- $x_{j} \ge 0$ - fraction of investment put into asset $j$ (decision variables)
- $r_{jt}$ - historical return of investment $j$ in moths $t$ - $t + 1$ (parameters)
...
upon in any way - the above model is almost linear in the decision variables. 
The only thing that needs to be taken care of is the absolute value in the 
objective function. But since it's in a minimization term, we can use the classic trick:

- $d_t \ge 0$ - absolute deviation from the expected mean return in months $t$ - $t + 1$ (auxillary variable)
...
Which means that after minimization each $d_t$ will become equal to the 
higher one of those values. \newline
...
x = m.addVars(data.N, lb=0, vtype=gb.GRB.CONTINUOUS)
d = m.addVars(data.T, lb=0, vtype=gb.GRB.CONTINUOUS)

m.addConstr(gb.quicksum(x[j] * data.av_r[j] for j in range(data.N)) >= B)
m.addConstr(gb.quicksum(x) == 1)
m.addConstrs(
    d[t] >= gb.quicksum(
        x[j] * (data.r[j,t] - data.av_r[j]) 
...
m.setObjective(gb.quicksum(d), gb.GRB.MINIMIZE)

m.optimize()

investments = sum(x_j.X > 0 for x_j in x.values())
######### END
```

And also I added an additional plot of the `investments` variable 
to report the necessary number of investments for each $\epsilon$.
...
portfolio.main(["indtrack6.dat"])
```

As expected - the higher the risk the higher the rewards. And also 
as expected - the more diversified the investments the lower the risk. 
Where (for $\epsilon = 1$) to get the highest reward one would 
need to invest everything into the one asset with the highest estimated 
mean return.
...
- $\max \min_t \sum\limits_j x_j r_{jt}$ - this can be linearized in a similar 
way we handled the Hamming distance problem. \newline

We introduce a variable that will represent the minimum return of the portfolio:

- $m = \min_t \sum\limits_j x_j r_{jt} \ge 0$ - because we're probably not 
interested in negative returns

And now we can maximize it in the objective function and add constraints 
to ensure that it actually is the minimum (smaller or equal than any other return) 
and that it satisfies the reward requirement ($m \ge B$).

$$
\begin{matrix}
    \max & m \\
    \forall_t & m \le \sum\limits_j x_j r_{jt} \\
    & m \ge B \\
    & \sum\limits_j x_j = 1 \\
    \forall_j & \ x_j \ge 0 \\
    & m \ge 0
...
- $\min \hat{\sigma}^2 = \sum\limits_i \sum\limits_j x_i x_j \hat{\sigma}_i \hat{\sigma}_j \rho_{ij}$

I don't see a way to linearize this, because of the $x_i x_j$ term. 
If $x$'s were binary then some tricks could be used, but not in this case.

\clearpage\newpage

## Subtask 8.e

When we're talking about whether or not a certain asset can be bought, 
we're unfortunattely stepping into binary variables. The easiest way 
to model such a situation is to introduce a variable:

- $y_j \in \{0, 1\}$ - is asset $j$ invested in (yes or no)

And then add a constraint to make sure that we can't invest in too many assets:

$$
\sum\limits_j y_j \le M
$$

And we also need to make sure that we cannot invest anything 
into unchosen assets:

$$
\forall_j x_j \le y_j
$$

So eg. the model from subtask 8.d would change like so:
...
The "indicator" variables added in te previous subtask make this 
additional feature very easy to implement as they can be used to 
"choose" which $x$'s should be constrained by the lower bound, so 
I will keep them. I've already explained in the previous subtask how 
to get to this model, so now I will only focus on the lower bound. \newline

All we need is to add a constraint which "turns off" for unchosen 
assets - eg.:

$$
\forall_j x_j \ge v y_j
$$

For $y_j = 0$ it's the same as the bound on $x_j$ and for $y_j = 1$ it's 
exactly what we're aiming for. \newline

So finally, to model this feature, the model from eg. subtask 8.d 
