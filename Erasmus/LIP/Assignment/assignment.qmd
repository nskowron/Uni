...
Now we want the objective function of $PR(\alpha, \beta, \gamma)$ 
to be always $\le$ than the original objective function. 
For that we need:
...
\begin{matrix}
    \begin{matrix}
        & \sum\limits_i\alpha_i (by_i - \sum\limits_j a_j x_{ij}) \le 0 \\
        \land & \sum\limits_j\beta_j (1 - \sum\limits_i x_{ij}) = 0 \\
        \land & \sum\limits_i\gamma_i (1 - y_i) \le 0
    \end{matrix}
    & \implies &
    \begin{matrix}
        \sum\limits_i c_i y_i \\
...
Which means that the maximum value (with respect to $\alpha, \beta, \gamma$ 
like stated above) of $opt(PR(\alpha, \beta, \gamma))$ is also $\le opt(P)$, 
therefore we can try maximizing $opt(PR(\alpha, \beta, \gamma))$ to "approximate" 
$opt(P)$ from below.
...
the solution, using some variables defined in the previous subtask:
...
We can easily extract the inequalities corresponding 
to the Gomory cuts by bringing the full tableau from 
the previous subtask into the initial form, where only 
the slack variables are in the basis:
...
Now we can clearly see the original constraints and the 
new ones, and we can plot them using any tool.
...
```{dot}
digraph {
    node [shape=record, fontname="Latin Modern Roman" ,fontsize=12]
